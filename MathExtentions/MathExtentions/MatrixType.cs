namespace MathExtentions
{
    namespace Matrix
    {
        public static class MatrixType
        {
            ...
            public static Matrix<Matrix<T>> Matrix<T>(int rows, int columns) => new Matrix<Matrix<T>>(rows, columns, (a, b) => a + b, (a, b) => a * b);
            public static Matrix<Matrix<T>> Matrix<T>(Matrix<T>[,] matrices) => new Matrix<Matrix<T>>(matrices, (a, b) => a + b, (a, b) => a * b);
        }
    }
}